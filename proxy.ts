...
export async function proxy (request:NextRequest) {
    try {
        ...
        // Rule - 2: User in trying to access reset password page
        ...
        // Rule - 4 User is Not logged in but tying to access protected route -> redirect to login 
        if(!accessToken || !isValidAccessToken){
            const logUrl = new URL('/login', request.url);
            logUrl.searchParams.set('redirect', pathWithQuery);
