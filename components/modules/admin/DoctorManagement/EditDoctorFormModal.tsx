...
import { Gender, IDoctor, IUpdateDoctorPayload } from "@/types/doctor.types"
import { ISpecialty } from "@/types/specialty.types"
import { editDoctorFormZodSchema, IEditDoctorFormValues } from "@/zod/doctor.validation"
...
const EditDoctorFormModal = ({
    ...
    const form = useForm({
        ...
        onSubmit: async ({value}) => {
            ...
            const specialtyChange: IUpdateDoctorPayload['specialties'] = []

            nextSpecialtyIds.forEach((specialtyId) => {
                if(!originalSpecialtyIds.has(specialtyId)){
                    specialtyChange.push({specialtyId, shouldDelete:false})
                }
            })

            originalSpecialtyIds.forEach((specialtyId) => {
                if(!nextSpecialtyIds.has(specialtyId)){
                    specialtyChange.push({specialtyId, shouldDelete:true})
                }
            })
            ...
            const payload: IUpdateDoctorPayload = {
                ...
                ...(specialtyChange.length > 0 ? {specialties: specialtyChange} : {}),
            }
            ...
            toast.success(result.message || 'Doctor update successfully')
            onOpenChange(false)
