...
import { IAppointment, IBookAppointmentPayload, IBookAppointmentResult } from "@/types/appointment.types"
...
export const bookAppointmentWithPayLater = async (payload: IBookAppointmentPayload) => {
    try {
        return await httpClient.post<IBookAppointmentResult>('/appointments/book-appointment-with-pay-later', payload)
    } catch (error) {
...
export const initiateAppointmentPayment = async (appointmentId: string) => {
    try {
        return await httpClient.post(`/appointments/initiate-payment/${appointmentId}`,{})
    } catch (error) {
...
export const getMySingleAppointment = async (appointmentId: string) => {
    try {
        return await httpClient.get(`/appointments/my-single-appointment/${appointmentId}`)
    } catch (error) {
